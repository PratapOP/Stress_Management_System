...
if run_scan:
    ...
    st.write(f"Estimated Neutral Emotion: {avg_face['neutral']:.2f}")
    st.write(f"Estimated Happy Emotion: {avg_face['happy']:.2f}")
    st.write(f"Estimated Sad Emotion: {avg_face['sad']:.2f}")

    # ---------- RECOMMENDATIONS ----------
    st.markdown('<p class="section-title">AI Recommendations</p>',
                unsafe_allow_html=True)
